...
/** Todas las escobas; solo 1 carta cara arriba por tramo (y 1 arriba del todo). */
...
function render(opts = {}) {
  ...
  wrap?.classList.toggle('deck-low', g.deck.length > 0 && g.deck.length <= 6);
  wrap?.classList.toggle('deck-empty', g.deck.length === 0);
  ...
  const layers = g.deck.length === 0 ? 0 : Math.min(4, Math.max(1, Math.ceil(g.deck.length / 10)));
  ...
  (deckHost || felt).appendChild(deckStack);
  ...
  // Mesa más grande con pocas; se reduce al llenarse
  const nTable = tableCards.length;
  let tableScale = 1.55;
  if (nTable >= 9) tableScale = 0.78;
  else if (nTable >= 7) tableScale = 0.9;
  else if (nTable >= 6) tableScale = 1.02;
  else if (nTable >= 5) tableScale = 1.2;
  else if (nTable >= 4) tableScale = 1.4;
